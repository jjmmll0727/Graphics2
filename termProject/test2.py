...
dst_pts = []
dst_pts_list = [] ##### 특징점들의 무게중심을 담는 배열 --> 물체 중앙에 마킹하기 위해서
dst_pts_list.append([])
...
def matching():
    ...
    while cap.isOpened():
        ...
        if roi_list[0] is None:  # 등록된 이미지 없음, 카메라 바이패스
            ...
            if len(good_matches) > MIN_MATCH*1: ###### 여기 곱하는 수를 조정해서 특징점의 범위 제한 가능
            ...
            for i in range(len(dst_pts)): ###### roi와 cap간의 특징벡터 매칭을 통해 나온 cap의 좌표(dst_pts)를 cap영상에 원으로 찍어준다.
                ...
                count = count + 1
                dst_mean = np.mean(dst_pts)
                dst_pts_x = dst_pts_x + dst_pts[i][0]
                dst_pts_y = dst_pts_y + dst_pts[i][1]
            print("gggggggggggg")
            print(dst_pts_x/count)
            print(dst_pts_y/count)
            #res = cv2.circle(img2, dst_mean, 3, (0, 255, 0), -1)

            #dst_pts_x = dst_pts_x / len(dst_pts) # 하나의 물체에 대한 특징점들의 무게중심 x좌표
            #dst_pts_y = dst_pts_y / len(dst_pts) # 하나의 물체에 대한 특징점들의 무게중심 y좌표
            #dst_list[0].append(dst_pts_x) # 모든 물체의 특징점들의 무게중심을 저장하고 나서 그 위치에 대해 한꺼번에 putText 하기 위해 이차원배열에 저장
            #dst_list[0].append(dst_pts_y)
            #res = cv2.circle(img2, tuple(dst_pts_list[0]), 5, (255,255,0), -1)
        # 결과 출력
